...
def push_to_gerrit(gerrit_url, gerrit_branch):
    try:
        ...
        subprocess.run(['git', 'push', gerrit_url, push_target], check=True)
...
if __name__ == "__main__":
    try:
        ...
        patch_dir = os.path.join(current_dir, config['output_dir'].strip("'\""))
        patch_list = ['test_amlrobot.patch']
        for patch in patch_list:
            ...
            if apply_patch(patch_path, config['code_base_dir'], config['gerrit_branch'].strip("'\"")):
                if commit_patch(patch_path):
                    ...
                    push_to_gerrit(push_url, config['gerrit_branch'])
